...
final class ReleaseMetadataTests: XCTestCase {
    func testPublicVersionIsZeroTwoTwo() {
        XCTAssertEqual(AppConstants.version, "0.2.2")
    }

    func testTrackedBundleMetadataUsesVersionZeroTwoTwoAndBuildSeven() throws {
        for path in [
            ...
            XCTAssertEqual(plist["CFBundleShortVersionString"] as? String, "0.2.2", path)
            XCTAssertEqual(plist["CFBundleVersion"] as? String, "7", path)
        }
    }

    func testCaskUsesVersionZeroTwoTwoAndRetainsARealChecksum() throws {
        let cask = try text("Casks/veloop.rb")
        XCTAssertTrue(cask.contains("version \"0.2.2\""))
        XCTAssertNotNil(
    ...
    func testSourceGuideDescribesSingleIdentityAndImmediateRefresh() throws {
        ...
        XCTAssertTrue(sourceGuide.contains("refreshes permission state every 100 milliseconds"))
        XCTAssertTrue(sourceGuide.contains("stops immediately when another application becomes active"))
        XCTAssertTrue(sourceGuide.contains("without restarting the Agent"))
        XCTAssertTrue(sourceGuide.contains("checks only until Accessibility recovery recreates the listener"))
        XCTAssertTrue(sourceGuide.contains("Accessibility is the only user-configured permission"))
        XCTAssertTrue(sourceGuide.contains("only Accessibility gates the UI and runtime"))
