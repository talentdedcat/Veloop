...
final class ReleaseMetadataTests: XCTestCase {
    func testPublicVersionIsZeroThreeOne() {
        XCTAssertEqual(AppConstants.version, "0.3.1")
    }
    ...
    func testTrackedBundleMetadataUsesCurrentReleaseVersion() throws {
        for path in [
            ...
            XCTAssertEqual(plist["CFBundleShortVersionString"] as? String, "0.3.1", path)
            XCTAssertEqual(plist["CFBundleVersion"] as? String, "10", path)
        }
    }

    func testCaskUsesVersionZeroThreeOneAndRetainsARealChecksum() throws {
        let cask = try text("Casks/veloop.rb")
        XCTAssertTrue(cask.contains("version \"0.3.1\""))
        XCTAssertNotNil(
            cask.range(of: #"sha256 "[0-9a-f]{64}""#, options: .regularExpression)
        )
        XCTAssertFalse(cask.contains(String(repeating: "0", count: 64)))
    }
