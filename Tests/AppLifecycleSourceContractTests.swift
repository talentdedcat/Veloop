...
final class AppLifecycleSourceContractTests: XCTestCase {
    ...
    func testModelQueriesFirstThenRecoversExactlyOnceWithoutPolling() throws {
        ...
        XCTAssertTrue(model.contains("permissionRefreshPending"))
        XCTAssertTrue(model.contains("restartAgentForPermissionRefresh"))
        XCTAssertTrue(model.contains("markPermissionRefreshPending"))
        XCTAssertTrue(model.contains("pendingPermissionGroup"))
        XCTAssertTrue(model.contains("maximumPermissionRefreshAttempts = 3"))
