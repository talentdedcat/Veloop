...
普通启动与激活时，Veloop 优先查询健康的 Agent，不会先重启。socket 每个阶段的截止时间为 200 ms，仅在查询失败后执行一次恢复。通过权限按钮打开“系统设置”后，返回 Veloop 时只会主动重启一次 Agent，等待新 socket 就绪并立即读取新的权限状态。这个重启是必要的，因为 macOS 只会让新进程使用刚添加的输入监控授权；Veloop 控制应用本身保持打开。
